...
/*   Updated: 2024/12/05 19:55:20 by vvobis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	scene_add_disk(void *data_ptr, void *null)
{
	...
	disk_new = (t_body){\
		.type = BODY_DISK,
		.id = id_set(ID_GROUP_DISC, data->scene.body_cursor), \
		.disk = (t_disk){\
			.point = (t_vector){\
			...
			.radius = 3}};
	disk_new.color = 0x20ff20;
	disk_new.texture = &data->scene.texture[0];
	disk_new.map = &data->scene.bump_map[0];
	scene_body_add(&disk_new, &data->scene);
}
...
void	scene_add_sphere_func(void *data_ptr, void *null)
{
	...
	sphere_new.map = &data->scene.bump_map[0];
	...
	plane_new = (t_body){\
		...
		.plane = (t_plane){\
			.point = (t_vector){\
				...
				.y = data->scene.camera.position.y, \
				.z = data->scene.camera.position.z + 10}, \
			.inverse_normal = (t_vector){0, 0, 0}, \
			.normal = (t_vector){.x = 0, .y = 0, .z = -1}}};
	plane_new.plane.inverse_normal = scale_vector(plane_new.plane.normal, -1);
	plane_new.color = 0x20ff20;
	plane_new.texture = &data->scene.texture[0];
	plane_new.map = &data->scene.bump_map[0];
...
void	scene_add_cylinder(void *data_ptr, void *null)
{
	...
	cylinder.map = &data->scene.bump_map[0];
