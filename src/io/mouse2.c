...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/04 15:22:12 by bszilas           #+#    #+#             */
/*   Updated: 2024/11/04 15:40:16 by bszilas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_grabbed_object(t_body *grabbed, double dx, double dy)
{
	...
	id_group = id_group_get(grabbed->id);
	if (id_group == ID_GROUP_SPHERE)
	{
		grabbed->sphere.center.x += dx / 10;
		grabbed->sphere.center.y -= dy / 10;
	}
	else if (id_group == ID_GROUP_PLANE)
	{
		grabbed->plane.normal.x += dx / 10;
		grabbed->plane.normal.y -= dy / 10;
	}
	else if (id_group == ID_GROUP_CYLINDER)
	{
		grabbed->cylinder.center.x += dx / 10;
		grabbed->cylinder.center.y -= dy / 10;
		calc_cyl_data(&grabbed->cylinder);
	}
}
